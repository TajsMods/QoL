...
const LOG_NAME = "TajsQoL:ContextMenu"
...
func _setup_color_picker() -> void:
    ...
    _color_picker.set_color(Color.WHITE)
    _color_picker.color_changed.connect(_on_custom_color_selected)
    _color_picker.color_committed.connect(func(_c): _close_color_picker())
...
func _get_sound():
    return Engine.get_main_loop().root.get_node_or_null("Sound")
