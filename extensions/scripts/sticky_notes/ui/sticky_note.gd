...
const EditGroupPopupExtension = preload("res://mods-unpacked/TajemnikTV-QoL/extensions/scripts/edit_group_popup.gd")
...
# Properties expected by edit_group_popup (duck typing interface)
...
func _update_title_icon() -> void:
	...
	if core != null:
		...
		if registry != null:
			var resolved: Dictionary = registry.resolve_icon(note_icon)
			if resolved.get("texture", null) != null:
				_title_icon.texture = resolved.texture
				return

	if note_icon.begins_with("res://") and ResourceLoader.exists(note_icon):
		_title_icon.texture = load(note_icon)
		return

	if note_icon.find(":") != -1:
		var relative := note_icon.split(":", false, 1)[1]
		if relative != "":
			var mapped_path := "res://textures/icons".path_join(relative)
			if ResourceLoader.exists(mapped_path):
				_title_icon.texture = load(mapped_path)
				return

	# Try to load from res://textures/icons by name
...
# Methods expected by edit_group_popup (duck typing interface)
...
func _open_edit_popup() -> void:
	_set_selected(true)
	# Use the static signal from our extension to bypass the type check in Signals.edit_group
	# The regular Signals.edit_group(node: WindowIndexed) fails for sticky notes
	if EditGroupPopupExtension.edit_sticky_note_signal != null:
		EditGroupPopupExtension.edit_sticky_note_signal.emit(self )
	else:
		# Fallback just in case, though it will likely fail silently if type check is strict
		Signals.edit_group.emit(self )
	_play_sound("click2")
