...
const SETTINGS_KEY := "tajs_qol.color_picker"
...
func _build_sv_plane(parent: Control):
	...
	_sv_shader = load("res://mods-unpacked/TajemnikTV-QoL/extensions/shaders/sv_plane.gdshader")
	if _sv_shader:
...
func _read_config_file() -> Dictionary:
	...
	if settings == null:
		return {}
	return settings.get_dict(SETTINGS_KEY, {})

func _write_config_file(data: Dictionary):
	var settings = _get_core_settings()
	if settings == null:
		return
	settings.set_value(SETTINGS_KEY, data)
