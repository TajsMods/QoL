...
func setup(settings = null, logger = null) -> void:
    _settings = settings
    _logger = logger
    _connect_signals()
...
func _connect_signals() -> void:
    ...
    # Signals autoload may not be available during early init
    if not is_instance_valid(Signals) or Signals == null:
        # Defer connection to next frame when autoloads are ready
...
func _on_connection_dropped(connection_id: String, connection_type: int, at: Vector2) -> void:
    _debug_log("connection_dropped signal received: id=%s, type=%d, at=%s" % [connection_id, connection_type, at])
    ...
    if not is_instance_valid(origin_resource):
        _log_warn("Origin resource no longer valid, canceling wire drop")
        return
    ...
    _check_and_emit_async(connection_id, connection_type, origin_shape, origin_color, is_output, at)


func _check_and_emit_async(connection_id: String, connection_type: int, origin_shape: String, origin_color: String, is_output: bool, drop_position: Vector2) -> void:
    ...
    if _is_mouse_over_connector(drop_position):
    ...
    if _is_mouse_over_any_window(drop_position):
    ...
    if drop_position == Vector2.ZERO:
        drop_position = _get_mouse_world_position()
...
func _is_mouse_over_connector(pos: Vector2) -> bool:
    ...
    var mouse_pos := pos if pos != Vector2.ZERO else Globals.desktop.get_global_mouse_position()
...
func _is_mouse_over_any_window(pos: Vector2) -> bool:
    if not is_instance_valid(Globals.desktop):
        return false
...
func _is_debug_enabled() -> bool:
    if _settings != null and _settings.has_method("get_bool"):
        return _settings.get_bool("core.debug", false)
    return false
