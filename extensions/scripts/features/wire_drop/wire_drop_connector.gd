...
var _filter # TajsCoreNodeCompatibilityFilter
...
func setup(core, logger = null) -> void:
    _core = core
    _logger = logger
...
func _log(message: String, is_error: bool = false) -> void:
    ...
    if _debug_callback.is_valid():
        _debug_callback.call(message, is_error)
...
func spawn_and_connect(window_id: String, position: Vector2, origin_info: Dictionary) -> bool:
    ...
    if limit != -1 and Globals.max_window_count >= limit:
        _notify("exclamation", "build_limit_reached")
        _play_sound("error")
        spawn_failed.emit("Build limit reached")
        return false

    if not Utils.can_add_window(window_id):
        _notify("exclamation", "Cannot add more of this window type")
        _play_sound("error")
    ...
    var connected := _auto_connect(window, origin_info)
    ...
    if connected:
        ...
        _notify("exclamation", "Node added (connection failed)")
