...
const SETTING_KEY := "tajs_qol.hide_claimed_requests"
...
func _ready() -> void:
	...
	if not Signals.request_claimed.is_connected(_on_request_claimed_mod):
...
func _init_settings() -> void:
	if Engine.has_meta(CORE_META_KEY):
		...
		if core and "settings" in core:
			_settings = core.settings
			_is_hiding_completed = _settings.get_value(SETTING_KEY, true)
...
func _on_hide_toggle_toggled(pressed: bool) -> void:
	...
	if _settings:
		_settings.set_value(SETTING_KEY, pressed)
	_update_request_visibility()
